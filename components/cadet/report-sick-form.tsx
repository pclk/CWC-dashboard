...
import { createReportSickRequest } from "@/actions/cadet-requests";
...
export function CadetReportSickForm() {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      const result = await createReportSickRequest({
    ...
    return (
      <section className="rounded-[2rem] border border-teal-200 bg-teal-50/70 p-6 shadow-sm">
        ...
        <h2 className="mt-2 text-2xl font-semibold text-slate-900">
          Your report sick request was submitted
        </h2>
        <p className="mt-2 text-sm leading-6 text-slate-700">
          Your instructor will review it next, then the CWC will give the final approval. You&apos;ll
          see the change reflected once it&apos;s approved.
        </p>
    ...
    >
      ...
      <div className="space-y-2">
        ...
        <textarea
          ...
          rows={3}
          placeholder="Diagnosis, clinic, or any context the instructor should know."
          className="w-full rounded-2xl border border-black/10 bg-white px-4 py-3 outline-none transition focus:border-teal-700"
        />
      </div>
      ...
      >
        {pending ? "Submitting..." : "Submit report sick request"}
      </button>
    </form>
  );
}
